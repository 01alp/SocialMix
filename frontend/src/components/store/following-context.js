      ...
      .then((data) => {
        ...
        if (!allChatItemArr || allChatItemArr.length === 0) {
          ...
          usersCtx.users &&
            ...
            !following &&
            setOtherListedChatUsers(usersCtx.users.filter((user) => user.public === 1)); // takes care if cur user is not following any user
          usersCtx.users &&
            usersCtx.users.length !== 0 &&
            following &&
            setOtherListedChatUsers(
              usersCtx.users.filter((user) => user.public === 1 && !following.some((followingUser) => followingUser.id === user.id))
            );
        ...
        if (allChatItemArr && allChatItemArr.length !== 0) {
          if (!following || following.length === 0) {
            ...
            const filteredPublicUsers = allPublicUsers.filter(
              (user) => !allOtherChatItems.some((chatItem) => user.id === chatItem.sourceid)
            );
            ...
            if (
              ...
              followingChatItems &&
              followingChatItems.length !== 0 &&
              (!filteredFollowingNoChatItems || filteredFollowingNoChatItems.length === 0)
            ) {
            ...
            if (following && following.length !== 0)
              allPublicUsersExcludeFolloiwing = usersCtx.users.filter(
                (user) => user.public === 1 && !following.some((followingUser) => followingUser.id === user.id)
              );
            ...
            if (filteredOtherNoChatItems && filteredOtherNoChatItems.length !== 0 && allOtherChatItems && allOtherChatItems.length !== 0) {
              ...
              (!filteredOtherNoChatItems || filteredOtherNoChatItems.length === 0) &&
              allOtherChatItems &&
